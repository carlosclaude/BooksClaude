...
const NODES = [
  ...
  // NEW in v8: GEB (Hofstadter)
  { id: "geb", label: "Gödel, Escher, Bach", layer: 1, pos: t(515, 310), description: "Hofstadter: formal self-reference as the engine of the 'I'. The strange loop — a system folding back on itself until meaning and selfhood emerge from a meaningless substrate. Form enacts content: fugal dialogues, paragraph-level acrostics, the Crab Canon reading identically forwards and backwards. Meaning lives at a LEVEL; zoom to the substrate and it vanishes (the ant, the neuron, MU-noise). The back half is most seductive, least rigorous — the leap from self-reference to consciousness is analogy doing a proof's work; the middle (TNT, MU, meaning crawling up out of symbol-shuffling) is where he does what nobody had. Hub node with few clean edges: formalization-of-systems is its own axis, and most novelists don't travel it. Layer 1 by the operative test — the idea drives the book, it doesn't decorate it." },
];
...
const EDGES = [
  ...
  // =====================================================================
  // ===== NEW in v8: GEB / HOFSTADTER neighborhood =====
  // =====================================================================
  { s: "geb", t: "borges", label: "completeness vs incompleteness", type: "tension", desc: "The Aleph and the Library dream totality — one point from which everything is seen. Gödel forbids exactly this: no consistent system powerful enough to see itself is complete. The Aleph is the anti-Gödel, the dream Gödel killed. The real Gödelian moment is the narrator's confession: the total vision exists, but successive language cannot enumerate it — a truth unreachable from inside the system. 'Library of Babel' is the tighter structural match." },
  { s: "geb", t: "borges", label: "Funes — abstraction as meaning", type: "parallel", desc: "Funes remembers everything and therefore cannot think: total particularity is the death of abstraction. The negative proof of Hofstadter's meaning-lives-at-a-level. Meaning IS lossy compression; perfect fidelity is anti-meaning. The 1942 proof of the hallucination worry — metaphor is productive infidelity, and Funes, like a zero-hallucination model, can't make one." },
  { s: "geb", t: "borges", label: "recursion / self-inclusion", type: "parallel", desc: "Self-reference as shared structure — the Aleph containing the Aleph, the Gödel sentence describing its own unprovability. The cleanest surface parallel, though the two diverge sharply on completeness (see the tension edge). Held as Parallel, not equation." },
  { s: "geb", t: "kafka", label: "undecidable — proved vs lived", type: "tension", desc: "Gödel proves the true-but-unprovable sentence exists, with the serenity of mathematics. Kafka strands you inside one: the Law real, total, and structurally unreachable from within. 'Before the Law' — a door made only for you that you die without entering. Same formal shape, opposite affect: cold proof vs lived condemnation. The same move as the Nabokov pairing — one structure, inverted register." },
  { s: "geb", t: "beckett", label: "self-generating system that can't halt", type: "parallel", desc: "The Unnamable: a voice trying to prove its own existence from inside a language that won't grant it, generating itself and unable to stop. A literary halting problem — the system can neither self-verify nor terminate from within. Self-reference as trap rather than serene structure." },
  { s: "geb", t: "pynchon", label: "?? living inside the system — undecidable as entropy ??", type: "tension", desc: "PROVISIONAL — do not develop until Gravity's Rainbow is finished. Hypothesis: Slothrop's 'is the Real Text real or am I drawing it?' as undecidability loaded with entropy — the system running down before the answer arrives. Distinct from Kafka's undecidable-as-condemnation. Node pinned on gut; edge unconfirmed." },
];
